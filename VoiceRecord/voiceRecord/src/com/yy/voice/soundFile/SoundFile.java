...
public class SoundFile {
	...
	public static SoundFile create(String fileName, ProgressListener progressListener)
			throws java.io.FileNotFoundException, java.io.IOException, InvalidInputException {
	...
	private void ReadFile(File inputFile) throws java.io.FileNotFoundException, java.io.IOException,
				...
				if (firstSampleData && format.getString(MediaFormat.KEY_MIME).equals("audio/mp4a-latm")
					...
					if (mProgressListener != null) {
						if (!mProgressListener.reportProgress((float) (tot_size_read) / mFileSize)) {
							// We are asked to stop reading the file. Returning
							// immediately. The
	...
	public void WriteFile(File outputFile, float startTime, float endTime) throws java.io.IOException {
		...
		while (true) {
			...
			if (outputBufferIndex >= 0 && info.size > 0 && info.presentationTimeUs >= 0) {
				...
				outputBuffers[outputBufferIndex].clear();
				codec.releaseOutputBuffer(outputBufferIndex, false);
				if (encodedBytes.remaining() < info.size) { // Hopefully this
															// should not
															// happen.
	...
	private void swapLeftRightChannels(byte[] buffer) {
		...
		if (buffer.length % 4 != 0) { // 2 channels, 2 bytes per sample (for one
										// channel).
			// Invalid buffer size.
			return;
		}
